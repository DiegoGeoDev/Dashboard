...
const createMarkersFromGeojson = (data) => {
	...
	function popUp(feature, layer) {
		const { properties } = feature;
		const { gender, salary, like } = properties;

		return layer.bindPopup(
			`
			<h1>Gender: ${gender === 'M' ? 'Men' : 'Women'}</h1>
			<h1>Remuneration: ${salary.toLocaleString('pt-br', { style: 'currency', currency: 'BRL' })}</h1>
			<h1>${like === 1 ? 'Likes' : 'Dislikes'} Japanese Food</h1>
			`,
